...
const int WALL_DISTANCE_FRONT = 15;
...
// gets ticks to signed so we can subtract
  long leftTicks = (long)getLeftTicks();
  long rightTicks = (long)getRightTicks();

// if left > right, slow left; if right > left, slow right
  int leftSpeed = baseSpeed;
  int rightSpeed = baseSpeed;
  
// positive difference > left ahead; negative > right ahead
  long diff = leftTicks - rightTicks; // difference

//limiting how much to adjust per loop so PWM wont spyke
  const long MAX_ADJUST = 30; //tune later this is just a try number

    if (diff > 0) {
    long adjust = diff;
    if (adjust > MAX_ADJUST) adjust = MAX_ADJUST;
    leftSpeed -= (int)adjust;    // slow the left wheel
  } else if (diff < 0) {
    long adjust = -diff;
    if (adjust > MAX_ADJUST) adjust = MAX_ADJUST;
    rightSpeed -= (int)adjust;   // slow the right wheel
  }
// valid PWM range
